...
Após a realização das simulações, os dados são resumidos em um \textit{DataFrame Pandas}, contendo os resultados mostrados na Tabela \ref{tab_data_frame}.
...
Sugeriu-se na Seção \ref{justificativa} (\textbf{Justificativa}), a partir de testes preliminares, que os principais gargalos do código do Zigue-Zague estavam na geração de infográficos e realização de simulações. Os dados na Tabela \ref{tab} confirmam essa percepção, já que as funções \textbf{simulação} e \textbf{imprimeCaminho} ocupam juntas  99,83\% do tempo de processamento do código. Portanto, o foco da análise do desempenho computacional será todo concentrado nestas duas funções.
