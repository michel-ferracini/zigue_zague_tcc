...
% Programa gprof2dot
\subsection{Programas \textit{gprof2dot} e \textit{SnakeViz}}
...
Para alcançar o primeiro objetivo proposto (realizar medições de tempo de execução nos códigos utilizados no Projeto Zigue-Zague - ver Subseção \ref{objetivos}), inicia-se o estudo com a geração de um grafo de todo o programa e, também, usa-se o programa \textit{SnakeViz} para gerar uma visualização detalhada de cada parte do mesmo.

Na geração do grafo, foi estabelecida uma poda de todos os ramos que utilizam 10\% ou menos do tempo de processamento, como ilustrado na Figura \ref{grafo:10}, utilizando o programa \textit{gprof2dot}.

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.7\linewidth]{img/grafo.png}
	\caption{Grafo do programa, com poda para 10\% ou menos do tempo de processamento.}
	\label{grafo:10}
\end{figure}

Note que no grafo (Figura \ref{grafo:10}) as funções \textbf{imprimeCaminho} e \textbf{simulacao} ocupam juntas 94,08\% do processamento. Do lado esquerdo, a função \textbf{imprimeCaminho} possui como filho o método \textbf{scatter} do módulo \textit{pyplot} da biblioteca \textit{matplotlib}, sendo chamado todas as vezes que se faz necessário imprimir um ponto na geração da visualização dos caminhos de mais alta probabilidades e também dos de mais baixas probabilidades no tabuleiro de jogo. Do lado direito, a função \textbf{simulação} chama repetidamente a função filha \textbf{jogar} que, por sua vez chama repetidamente dois filhos: o método \textbf{sample} da biblioteca \textit{random} e a função \textbf{resultadosExpressoes}.

Na visualização gerada com o \textit{SnakeViz} (Figura \ref{viewSnakeViz}), os retângulos maiores indicam as funções que demandam mais tempo de execução e retângulos adjacentes à borda inferior de um certo retângulo, indicam funções filhas, de modo análogo ao grafo.

\begin{figure}[ht]
	\centering
	\includegraphics[width=1\linewidth]{img/view_snakeviz_1.png}
	\caption{Grafo do programa, com poda para 10\% ou menos do tempo de processamento.}
	\label{viewSnakeViz}
\end{figure}
