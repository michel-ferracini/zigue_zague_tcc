...
O programa \textit{SnakeViz}, como descrito em \cite{SnakeViz:2022}, é um visualizador gráfico baseado em navegador para a saída do módulo cProfile do \textit{Python} e uma alternativa ao uso do módulo \textit{pstats} da biblioteca padrão.

 

% Biblioteca NumPy
\subsection{Bibliotecas \textit{NumPy}}
...
Para alcançar o primeiro objetivo proposto (realizar medições de tempo de execução nos códigos utilizados no Projeto Zigue-Zague - ver Subseção \ref{objetivos}), inicia-se o estudo com a geração de um grafo, onde o conjunto de nós é constituído tanto pelas funções que integram o código quanto pelas variáveis retornadas por várias destas funções. A ideia é mapear a relação de interdependência entre funções e variáveis, possibilitando \aspas{percorrer} o código de modo mais eficiente na busca por alguma eventual abordagem lógica inadequada.

\subsection{Diagrama dos Métodos de Mapeamento do Tabuleiro}

Na Figura \ref{diagrama_mapeamento}, os retângulos em azul representam os métodos; os trapézios em amarelo e os paralelogramos em verde indicam os parâmetros dos métodos, sendo que os primeiros indicam parâmetros escolhidos e os últimos são parâmetros calculados a partir de outros métodos. As linhas seccionadas relacionam os parâmetros aos métodos, indicando quais parâmetros cada método utiliza.

\begin{figure}[ht]
	\centering
	\includegraphics[width=1\linewidth]{img/diagrama_mapeamento.png}
	\caption{Diagrama das funções utilizadas para mapear o tabuleiro.}
	\label{diagrama_mapeamento}
\end{figure}
